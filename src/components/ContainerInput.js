...
const ContainerInput = ({
  ...
  const handleSelect = (percentage) => calculator(percentage);

  const handleCustom = (percentage) => calculator(percentage);

  const calculator = (percentage) => {
    if (bill <= 0) {
      document.getElementById("bill").classList.add("row-input-error");
      setErrorBill(true);
    }
    if (people <= 0) {
      document.getElementById("people").classList.add("row-input-error");
      setErrorPeople(true);
    }
  ...
  const handleBill = (value) => {
    const nodo = document.getElementById("bill");
    if (nodo.classList.contains("row-input-error")) {
      nodo.classList.remove("row-input-error");
      setErrorBill(false);
    }
    setBill(value);
  };
  ...
  const handlePeople = (value) => {
    const nodo = document.getElementById("people");
    if (nodo.classList.contains("row-input-error")) {
      nodo.classList.remove("row-input-error");
    ...
    <div className="container-input">
      <div className="sub-title">
        <div style={{ display: "flex", position: "relative" }}>
          <p>Bill</p>
          {errorBill && <InputError />}
        </div>
        <div className="form-control">
          <div id="bill" className="row-input">
      ...
      <div className="sub-title">
        ...
        <div className="selects">
          <button type="button" onClick={() => handleSelect(5)}>
            5%
          </button>
          <button type="button" onClick={() => handleSelect(10)}>
            10%
          </button>
          <button type="button" onClick={() => handleSelect(15)}>
            15%
          </button>
          <button type="button" onClick={() => handleSelect(25)}>
            25%
          </button>
          <button type="button" onClick={() => handleSelect(50)}>
            50%
          </button>
          <input
            type="number"
      ...
      <div className="sub-title">
        <div style={{ display: "flex", position: "relative" }}>
          ...
          {errorPeople && <InputError />}
        </div>
        <div className="form-control">
          <div id="people" className="row-input">
